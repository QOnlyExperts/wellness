...
export function configSocket(server: HttpServer): Server {
  ...
  const admins = new Set<string>(); // IDs de administradores
  const clients = new Set<string>(); // IDs de clientes
  ...
  io.on('connection', (socket: Socket) => {
    ...
    socket.on('joinAsAdmin', async (user: any) => {
      const result = { res: { rol: 'admin' } };

      if (result.res.rol !== 'admin') return;
    ...
    // 🔹 Unirse como cliente
    socket.on('joinAsClient', async (user: any) => {
      const result = { res: { rol: 'student' } };

      if (result.res.rol !== 'student') return;

      clients.add(socket.id);
      socket.join('clientRoom');
    });

    // 🔹 Cliente solicita instrumento al admin
    socket.on('requestInstrumentToAdmin', async (data: any) => {
      const { idInstrument, user } = data;
    ...
    socket.on('responseToClient', async (data: any) => {
      const { id, requ, user } = {
        id: 1,
        requ: 1,
    ...
    // 🔹 Eliminar instrumento en uso
    socket.on('deleteInstrumentInUse', async (data: any) => {
...
export function getSocket(): Server {
  if (!io) {
    throw new Error('Socket.io no configurado. Asegúrate de llamar a configSocket primero.');
  }
  return io;
}
